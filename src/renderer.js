...
const COLS = Math.min(process.stdout.columns || 72, 80);
const BAR_MAX = 26;
...
function bar(pct, color = 'cyan', max = BAR_MAX) {
  ...
  return chalk[color]('█'.repeat(Math.max(0, filled))) + chalk.gray('░'.repeat(Math.max(0, empty)));
}
...
export function render({ user, languages, timing, stats, dnaTraits, commitStyle, activity }) {
  ...
  console.log(`  ${chalk.gray('★')} ${chalk.white(stats.totalStars)} stars  ·  ${chalk.white(user.public_repos)} repos  ·  since ${joined}${loc}`);
  ...
  for (const { lang, pct, repos } of languages) {
    ...
    console.log(`  ${chalk.gray(label)} ${bar(pct, langColor(lang))}  ${chalk.white(pctStr)}  ${repoStr}`);
  }
  ...
  for (const h of slots) {
    const count = timing.hours[h] + (timing.hours[h + 1] || 0) + (timing.hours[h + 2] || 0);
    const isPeak =
      h === timing.peakHour ||
      (h + 1) === timing.peakHour ||
      (h + 2) === timing.peakHour;
  ...
  const isMobile = ['Swift', 'Kotlin', 'Dart', 'Objective-C'].includes(topLang);
  const isPolyglot = languages.length;
  const isNight = timing.night > timing.morning && timing.night > timing.afternoon;
  const openness = stats.totalStars;

  const traits = [
    { label: 'Mobile-first', value: isMobile ? 100 : (languages.filter(l => ['Swift', 'Kotlin', 'Dart'].includes(l.lang)).reduce((a, l) => a + l.pct, 0)), max: 100 },
    { label: 'Polyglot', value: Math.min(isPolyglot * 16, 100), max: 100 },
    { label: 'Night owl', value: isNight ? Math.min(timing.night * 10, 100) : Math.min(timing.evening * 8, 100), max: 100 },
    { label: 'Open source', value: Math.min(openness * 2, 100), max: 100 },
    { label: 'Active', value: Math.min(activity.recentPushCount * 4, 100), max: 100 },
  ];

  for (const t of traits) {
    const label = t.label.padEnd(14);
    const pct = Math.round(Math.min(t.value, 100));
    const pctStr = (pct + '%').padStart(4);
    console.log(`  ${chalk.gray(label)} ${bar(pct, 'green')}  ${chalk.white(pctStr)}`);
  }
  ...
  for (const repo of stats.topRepos) {
    ...
    const forks = repo.forks_count > 0 ? chalk.gray(` · ⑂ ${repo.forks_count}`) : '';
    console.log(`  ${chalk.bold.white(repo.name)}  ${stars}${lang}${forks}`);
    if (repo.description) {
      const desc = repo.description.length > COLS - 6
