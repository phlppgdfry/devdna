...
const FRONTEND_LANGS = new Set(['JavaScript', 'TypeScript', 'CSS', 'HTML', 'Vue', 'Svelte']);
const BACKEND_LANGS = new Set(['Go', 'Rust', 'Java', 'Python', 'Ruby', 'PHP', 'C#', 'C++', 'C', 'Elixir', 'Haskell', 'Scala']);
...
export function analyze({ user, repos, events }) {
  ...
  const dnaTraits = computeDNATraits({ user, languages, timing, stats, activity });
  const commitStyle = computeCommitStyle(events);

  return { user, languages, timing, activity, stats, dnaTraits, commitStyle };
}
...
function analyzeLanguages(repos) {
  ...
  for (const repo of repos) {
    ...
    bytes[repo.language] = (bytes[repo.language] || 0) + Math.max(repo.size || 1, 1);
...
function analyzeTiming(events) {
  ...
  return { hours, days, peakHour, pushCount, morning, afternoon, evening, night };
}

function analyzeActivity(events) {
  const recentPushes = events.filter(e => e.type === 'PushEvent');
  const repos = new Set(recentPushes.map(e => e.repo?.name));
  ...
  return { recentPushCount: recentPushes.length, activeRepos: repos.size, stars, forks, prs };
}
...
function analyzeStats(repos) {
  ...
  const totalForks = original.reduce((a, r) => a + r.forks_count, 0);
  const totalWatchers = original.reduce((a, r) => a + r.watchers_count, 0);
  ...
  return { totalStars, totalForks, totalWatchers, topRepos, topTopics, originalCount: original.length };
}
...
function computeDNATraits({ user, languages, timing, stats, activity }) {
  ...
  // TIME TRAITS
  const maxPeriod = Math.max(timing.morning, timing.afternoon, timing.evening, timing.night);
  if (maxPeriod === timing.night) traits.push('Night Owl');
  ...
  else if (topLang && BACKEND_LANGS.has(topLang)) traits.push('Backend Pragmatist');
...
function computeCommitStyle(events) {
  ...
  const emojiCount = messages.filter(m => /[\u{1F300}-\u{1FFFF}]/u.test(m)).length;
  const fixCount = messages.filter(m => /\bfix\b|\bbug\b/.test(m)).length;
  const wip = messages.filter(m => /\bwip\b|\btemp\b|\btest\b|\btodo\b/.test(m)).length;
